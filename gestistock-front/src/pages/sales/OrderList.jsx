...
import { Plus, Eye, XCircle, FileText, ShoppingCart } from 'lucide-react'
...
export default function OrderList() {
  ...
  const fetchOrders = async (page = 1) => {
    try {
      setLoading(true)
      const res = await orderService.getAll({ ...filters, page })
  ...
  return (
    <AppLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Commandes clients</h1>
          <p className="text-sm text-gray-500 mt-1">{pagination.total || 0} commande(s)</p>
        </div>
        {canManage && (
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 mb-6 flex flex-wrap gap-3">
        ...
        <input type="date" className="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      {canManage && ['brouillon', 'confirme'].includes(order.status) && (
